...
class LongTermMemory:
            ...
            try:
                ...
                conn.execute(
                    ...
                    CREATE TABLE IF NOT EXISTS relations (
                        ...
                        FOREIGN KEY (source_entry_id) REFERENCES entries(entry_id)
                    )
                    """
                )
                conn.execute(
                ...
                conn.execute(
                    """
                    CREATE INDEX IF NOT EXISTS idx_relations_entry ON relations(source_entry_id)
                    """
                )
                conn.commit()
            finally:
    ...
    def insert_entity(self, entity_id: str, name: str, entity_type: str = "other") -> bool:
        """插入或忽略实体记录。"""
        with self._lock:
            conn = self._connect()
            try:
                conn.execute(
                    """
                    INSERT OR IGNORE INTO entities (entity_id, name, type, metadata_json)
                    VALUES (?, ?, ?, '{}')
                    """,
                ...
                logger.debug(f"[LongTermMemory] insert_entity failed: {e}")
                return False
            finally:
                conn.close()
    ...
    def insert_relation(
        ...
        source_entry_id: str, confidence: float = 1.0,
    ) -> bool:
        """插入关系记录。"""
        with self._lock:
            conn = self._connect()
            try:
                conn.execute(
                    ...
                    (subject_id, predicate, object_id, source_entry_id, confidence)
                    VALUES (?, ?, ?, ?, ?)
                    """,
                    (subject_id, predicate, object_id, source_entry_id, confidence),
                )
                ...
                logger.debug(f"[LongTermMemory] insert_relation failed: {e}")
                return False
            finally:
                conn.close()

    def get_entities_for_entry(self, entry_id: str) -> list[str]:
        """获取某个 entry 关联的所有实体 ID。"""
        with self._lock:
            conn = self._connect()
            try:
                cur = conn.execute(
                    """
                    SELECT DISTINCT subject_id AS entity_id FROM relations
                    WHERE source_entry_id = ?
                    UNION
                    SELECT DISTINCT object_id AS entity_id FROM relations
                    WHERE source_entry_id = ?
                    """,
                    (entry_id, entry_id),
                )
    ...
    def get_entries_for_entity(self, entity_id: str) -> list[str]:
        """获取某个实体关联的所有 entry ID。"""
        with self._lock:
            conn = self._connect()
            try:
                cur = conn.execute(
                    """
                    SELECT DISTINCT source_entry_id FROM relations
                    WHERE subject_id = ? OR object_id = ?
                    """,
                    (entity_id, entity_id),
                )
                return [r["source_entry_id"] for r in cur.fetchall()]
            finally:
                conn.close()

    def get_entity_relations(self, entity_id: str) -> list[dict]:
        """获取某个实体的所有关系（包含邻居实体 ID）。"""
        with self._lock:
            conn = self._connect()
            try:
                cur = conn.execute(
                    """
                    SELECT subject_id, predicate, object_id, source_entry_id, confidence
                    FROM relations
                    WHERE subject_id = ? OR object_id = ?
                    """,
                    (entity_id, entity_id),
                ...
                results = []
                for r in cur.fetchall():
                    neighbor = r["object_id"] if r["subject_id"] == entity_id else r["subject_id"]
                    results.append({
    ...
    def get_entity_by_name(self, name: str) -> list[dict]:
        ...
        with self._lock:
            ...
            try:
                ...
                return [{"entity_id": r["entity_id"], "name": r["name"], "type": r["type"]}
                        for r in cur.fetchall()]
            finally:
                conn.close()
