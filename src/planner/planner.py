...
from ..utils.structured_output import generate_structured, PLANNER_DAG_SCHEMA
...
class Planner:
    ...
    def generate_plan(self, query: str, memory_context: str = "") -> DAG:
        ...
        # 优先使用约束解码，失败时回退到普通调用 + 正则解析
        data = None
        try:
            data = generate_structured(
                self.policy, messages, PLANNER_DAG_SCHEMA,
        ...
        if data is None:
            ...
            self._last_raw_json = json.dumps(data, ensure_ascii=False)
            self.budget_tracker.track(len(self._last_raw_json) // 3)
    ...
    def _build_dag_from_data(self, data: dict[str, Any]) -> DAG:
        """从结构化 dict 构建 DAG（用于约束解码直接返回的 JSON）。"""
