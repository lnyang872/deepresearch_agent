...
class ResearcherAgent(BaseAgent):
        ...
        for turn in range(self.max_turns):
            ...
            if tool_errors and not tool_results:
                return AgentResult(
                    task_id=task.task_id,
                    status=AgentStatus.FAILED,
                    output=f"All tools failed: {'; '.join(tool_errors)}",
                    trajectory=trajectory,
                    token_usage=total_tokens,
                    confidence=0.0,
                )
    ...
    async def _execute_tools_parallel(
        ...
        使用 asyncio.gather 并发执行，Semaphore 限流。
        单个工具失败不阻塞其他工具，汇总后返回。

        Returns:
            (tool_results, tool_errors): 成功的工具结果列表和失败信息列表。
        """
        async def _execute_one(tc) -> tuple[dict | None, str | None]:
            """执行单个工具调用，返回 (result_dict, error_string)。"""
            async with self._tool_semaphore:
                ...
                try:
                    ...
                    err = f"{tool_name}: {type(e).__name__}: {e}"
                    trajectory.append({
                        "turn": turn,
                        "role": "tool",
        ...
        for item in gathered:
            if isinstance(item, Exception):
                tool_errors.append(f"Internal: {type(item).__name__}: {item}")
            elif item is not None:
                result_dict, error_str = item
