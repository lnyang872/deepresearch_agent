...
from src.utils.structured_output import generate_structured, RED_AGENT_SCORE_SCHEMA
            ...
            try:
                ...
                try:
                    data = generate_structured(
                        self.policy, messages, RED_AGENT_SCORE_SCHEMA,
                        schema_name=f"red_agent_{dim.value}",
                    )
                    score, issues = self._extract_from_dict(data, dim)
                except Exception:
                    # 约束解码失败 → 回退到普通调用 + 正则解析
                ...
                raw_feedbacks.append(f"[{dim.value}] score={score:.1f}, issues={len(issues)}")
